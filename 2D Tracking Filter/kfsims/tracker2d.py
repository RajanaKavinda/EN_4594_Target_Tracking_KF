...
def run_sim(KalmanFilterModel, sim_option, kf_options):
    ...
    if draw_animation is True:
        ...
        def update_plot(i):
            if i < len(vehicle_position_history):
                if isinstance(vehicle_position_history[i], (list, tuple)) and len(vehicle_position_history[i]) == 2:
                    x, y = vehicle_position_history[i]
                    if isinstance(x, (int, float)) and isinstance(y, (int, float)):
                        vehicle_plot.set_data([x], [y])  # Update plot with valid data
                    else:
                        print(f"Invalid types in vehicle_position_history[{i}]")
                else:
                    print(f"Invalid format for vehicle_position_history[{i}]")
            ...
            if estimated_state_history[i] is not None:
                est_xpos, est_ypos = estimated_state_history[i][:2]
                if isinstance(est_xpos, (int, float)) and isinstance(est_ypos, (int, float)):
                    estimate_plot.set_data([est_xpos], [est_ypos])  # Update with valid data
                ...
                if estimated_covariance_history[i] is not None:
                    try:
                        ...
                        print(f"Error processing covariance for frame {i}: {e}")
